...
test("form header renders", () => {
  ...
  test("form shows success message on submit with form details", async () => {
    ...
    fireEvent.change(firstName, { target: { value: "Brittany" } });
    fireEvent.change(lastName, { target: { value: "Canty" } });
    fireEvent.change(address, { target: "123 Main Street" });
    fireEvent.change(city, { target: { value: "Nashville" } });
    fireEvent.change(state, { target: { value: "TN" } });
    fireEvent.change(zip, { target: { value: "37213" } });
